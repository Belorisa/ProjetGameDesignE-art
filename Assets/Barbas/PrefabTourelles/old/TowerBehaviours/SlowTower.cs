...
public class SlowTower : StateTowerLevelUp
{
    public TowerSlow tower;

    public GameObject closestTarget;

    public float fireTimer;

    public SlowTower(TowerSlow tower) : base(tower)
    {
        this.tower = tower;
    }
        ...
        if (closestTarget != null)
        {
            if (fireTimer <= 0)
            {
                ...
                oui.GetComponent<Bullet>().myStats = new BulletStats(tower.myStats.damages, tower.myStats.bulletSpeed,
                    new SlowBullet(oui.GetComponent<Bullet>(), tower.mySpecialStats.slow, tower.mySpecialStats.slowTimer), closestTarget);
    ...
    public override void OnLevelUp(int i)
    {
            ...
            tower.gears.GetComponent<Gears>().playerActionManager.GetComponent<PlayerActionManager>().golds >= tower.statsForEachLevel[i].goldCost)
        {
            ...
            tower.mySpecialStats = tower.speciaStatesPerLevel[i]; //just pour la tourelle slow
